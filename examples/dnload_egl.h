#ifndef SHRINKY_EGL_H
#define SHRINKY_EGL_H
...
{
  EGLBoolean result = shrinky_eglTerminate(egl_display);
#if defined(USE_LD)
  if(!result)
  {
...
static EGLBoolean egl_init(EGLNativeWindowType native_window, EGLDisplay *out_display, EGLSurface *out_surface)
{
  ...
  display = shrinky_eglGetDisplay(EGL_DEFAULT_DISPLAY);
#if defined(USE_LD)
  ...
  result =   shrinky_eglInitialize(display, NULL, NULL);
#if defined(USE_LD)
  if(!result)
  {
  ...
  result =   shrinky_eglChooseConfig(display, desired_config, &acquired_config, 1, &config_count);
#if defined(USE_LD)
  ...
  surface =   shrinky_eglCreateWindowSurface(display, acquired_config, native_window, NULL);
#if defined(USE_LD)
  ...
  context =   shrinky_eglCreateContext(display, acquired_config, EGL_NO_CONTEXT, context_attributes);
#if defined(USE_LD)
  ...
  result =   shrinky_eglMakeCurrent(display, surface, surface, context);
#if defined(USE_LD)
  if(!result)
  {
    egl_terminate(display);
...
{
  EGLBoolean result =  shrinky_eglMakeCurrent(display, EGL_NO_CONTEXT, EGL_NO_CONTEXT, EGL_NO_CONTEXT);
#if defined(USE_LD)
  if(!result)
  {
    fprintf(stderr, "eglMakeCurrent(): %i\n", eglGetError());
