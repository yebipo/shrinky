#ifndef SHRINKY_VIDEOCORE_H
#define SHRINKY_VIDEOCORE_H
...
static void videocore_open_native_window(const VC_RECT_T *dst_rect, const VC_RECT_T *src_rect)
{
  ...
  DISPMANX_DISPLAY_HANDLE_T dispman_display = shrinky_vc_dispmanx_display_open(0);
  DISPMANX_UPDATE_HANDLE_T dispman_update = shrinky_vc_dispmanx_update_start(0);
  DISPMANX_ELEMENT_HANDLE_T element = shrinky_vc_dispmanx_element_add(
  ...
  result = shrinky_vc_dispmanx_update_submit_sync(dispman_update);
#if defined(USE_LD)
  if(result)
  {
...
static void videocore_create_native_window_extended(int screen_width, int screen_height, enum WindowMode mode)
{
  ...
  shrinky_bcm_host_init();

  result = shrinky_graphics_get_display_size(0, &native_width, &native_height);
#if defined(USE_LD)
  if(result)
  {
...
static void videocore_move_native_window(int px, int py, int width, int height)
{
  ...
  shrinky_vc_dispmanx_update_submit_sync(dispman_update);
}
#endif
